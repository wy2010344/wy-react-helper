...
export type SvgElement<T extends SvgElementType> =
  JSX.IntrinsicElements[T] extends React.SVGProps<infer N> ? N : never;
export type SvgAttribute<T extends SvgElementType> = SVGAttributes<
  SvgElement<T>
>;

export type SvgAttributeS<T extends SvgElementType> = Omit<
  SvgAttribute<T>,
  'style'
> & {
  style: CSSProperties;
};
export type SvgAttributeSO<T extends SvgElementType> = Omit<
  SvgAttribute<T>,
  'style'
> & {
  style?: CSSProperties;
...
export const svg: {
  readonly [key in SvgElementType]: {
    (
      props?: SvgAttribute<key> | SvgAttributeSO<key>,
      portal?: Node
    ): DomCreater<SvgAttribute<key>, key>;
    (
