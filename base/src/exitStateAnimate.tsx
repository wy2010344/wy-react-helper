...
const useCacheList = createUseReducer(function <V>(
  list: readonly CacheModel<V>[],
  action:
    | {
        method: 'replace';
  ...
  if (action.method == 'remove') {
    return list.filter(
      v =>
        v.state == 'exit' && v.key == action.key && v.promise == action.promise
    );
  }
  return list;
});
...
export function useExitStateAnimate<V>(
  ...
  const [cacheList, dispatch] = useCacheList(
    emptyArray as unknown as readonly CacheModel<V>[]
  );
  ...
  function mergeAddList(
    ...
    for (const v of list) {
      ...
      const oldIndex = oldList().findIndex(
        old => old.key == key && old.state != 'exit'
      );
      if (oldIndex < 0) {
        //尚且不存在
        if (mode == 'shift') {
  ...
  useEffect(() => {
    ...
    if (cachesH.isDirty()) {
      ...
      if (onAnimationComplete) {
        Promise.all([...enterPromiseList, exitPromiseList]).then(
          onAnimationComplete
        );
      }
    }
